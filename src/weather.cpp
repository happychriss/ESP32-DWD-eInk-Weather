...
bool downloadKML(const String fetch_url) {
...
//    Serial.printf("Fetching URL: %s\n", url.c_str());
    ...
    if (!mz_zip_reader_init_file(&zip_archive, MOSMIX_ZIP_FILE, 0)) {
    ...
    String outputPath = "/spiffs/" + String(file_stat.m_filename);

#undef SKIP

#ifndef SKIP

    DPL("Extracting the file...");
    if (!mz_zip_reader_extract_to_file(&zip_archive, 0, MOSMIX_FILE, 0)) {
        DP("Error extracting the file: ");
        mz_zip_error mzerr = mz_zip_get_last_error(&zip_archive);
        DPL(mz_zip_get_error_string(mzerr));
        mz_zip_reader_end(&zip_archive);
        return false;
    }
    DPL("Extracted the file.");
    mz_zip_reader_end(&zip_archive);
#endif
...
std::pair<int, int> get_hour_index_for_time(std::tm *forecast_time, pugi::xml_node dt_root) {
    ...
    for (pugi::xml_node dt: dt_root.children("dwd:TimeStep")) {
        ...
        if (b_found) {
            HourlyWeather[update_index].time = TimeStep;
            HourlyWeather[update_index].hour = hours_index;
            update_index++;
        }
...
void getWeather() {
    ...
    if (downloadKML(DWD_URL)) {

        pugi::xml_document dwd_xml;
        if (dwd_xml.load_file(MOSMIX_FILE)) {
...
//            DPF( "dforecast_root: %s\n", dforecast_root.name());
            ...
            for (int i = 0; i < forecast_value.size(); ++i) {
                ...
                HourlyWeather[i].forecast = getWeatherString((int) forecast_value[i]);
                HourlyWeather[i].forecast_id = (int) forecast_value[i];
            ...
            for (int i = 0; i < forecast_value.size(); ++i) {
                ...
                HourlyWeather[i].temperature = forecast_value[i] - 273.15;
            }
            ...
            for (int i = 0; i < forecast_value.size(); ++i) {
                // DPF( "Hour %d: %f mm\n", i, forecast_value[i]);
                HourlyWeather[i].rain = forecast_value[i];
            }
            ...
            for (int i = 0; i < forecast_value.size(); ++i) {
                ...
                // DPF( "Hour %d: %f Bft\n", i, wind_beaufort);
                HourlyWeather[i].wind = wind_beaufort;
            }
            ...
            for (int i = 0; i < forecast_value.size(); ++i) {
                HourlyWeather[i].sun = static_cast<double >(std::round(forecast_value[i] / 60));
            }
            ...
            for (int i = 0; i < forecast_value.size(); ++i) {
                HourlyWeather[i].clouds = forecast_value[i];
            }

            for (auto &hw: HourlyWeather) {
                printHourlyWeather(hw);
            }


        } else {
            DP("ERROR: Failed to load and parse the KML file.");
        }
